...
public class RouterU {
    public static char id = 'u';
    public static int inf = 10000;
    public static int port = PortMap.map.get(id); // 8000
    public static int broadcastNo = 1;
    public static int broadcastThreshold = 5; // After 5 broadcast, cost will change

    public static int[][] cost = {
            //        u    v    w    x    y    z
            /* u */ { 0 ,  2 ,  5 ,  1 , inf, inf},
            /* v */ { 2 ,  0 , inf, inf, inf, inf},
            /* w */ { 5 , inf,  0 , inf, inf, inf},
            /* x */ { 1 , inf, inf,  0 , inf, inf},
            /* y */ {inf, inf, inf, inf,  0 , inf},
            /* z */ {inf, inf, inf, inf, inf,  0 }
    };
...
//        printCost();
        ...
        DatagramSocket socket = new DatagramSocket(port); // UDP SERVER at PORT 8000
        System.out.println("Broadcasting"); broadcast(socket); System.out.println("Broadcasting Ended");
        ...
        while(true){
            ...
            if(src != '-' && dst != '-'){
                ...
                for(int i=0; i < cost[idIdx].length; i++){
                    ...
                    if(linkCost < cost[(int)src - (int)'u'][(int)dst - (int)'u'] || linkCost < cost[(int)dst - (int)'u'][(int)src - (int)'u']){
